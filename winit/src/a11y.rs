...
use crate::core::widget::operation::accessible::{
    Accessible, Live as IcedLive, Orientation as IcedOrientation,
...
use accesskit::{
    ...
    Node, NodeId, Role, Toggled, Tree, TreeUpdate,
};
...
impl A11yAdapter {
    ...
    pub(crate) fn update_if_active(
        &mut self,
        update_fn: impl FnOnce() -> TreeUpdate,
    ) {
...
pub(crate) fn synthetic_arrow_key(
    center: Point,
...
impl TreeBuilder {
    ...
    fn alloc_id(&mut self, widget_id: Option<&widget::Id>) -> NodeId {
        ...
        if let Some(wid) = widget_id {
            ...
            let sibling_count =
                self.children.get(&parent).map_or(0, Vec::len) as u64;
    ...
    pub fn build(mut self) -> A11yTree {
        ...
        let wid_to_node: HashMap<widget::Id, NodeId> = self
            .node_map
            .iter()
            .filter_map(|(nid, (wid, _))| {
                wid.as_ref().map(|w| (w.clone(), *nid))
            })
            .collect();
        ...
        for (source_id, target_wid) in &self.label_refs {
            if let Some(target_nid) = wid_to_node.get(target_wid)
                && let Some((_, node)) =
                    self.nodes.iter_mut().find(|(nid, _)| nid == source_id)
            {
...
impl Operation for TreeBuilder {
    ...
    fn text(&mut self, id: Option<&widget::Id>, bounds: Rectangle, text: &str) {
        ...
        if let Some(acc_id) = self.current_accessible
            && let Some((_, node)) =
                self.nodes.iter_mut().find(|(nid, _)| *nid == acc_id)
    ...
    fn focusable(
        ...
        if let Some(node_id) = self.current_accessible {
            if let Some((_, node)) =
                self.nodes.iter_mut().find(|(nid, _)| *nid == node_id)
            {
    ...
    fn scrollable(
        ...
        if let Some(parent_id) = self.current_accessible
            && let Some((_, parent_node)) =
                self.nodes.iter_mut().find(|(nid, _)| *nid == parent_id)
        {
            parent_node.set_scroll_y(translation.y as f64);
            parent_node.set_scroll_y_min(0.0);
            parent_node.set_scroll_y_max(
                (content_bounds.height - bounds.height).max(0.0) as f64,
            );
            parent_node.set_scroll_x(translation.x as f64);
            parent_node.set_scroll_x_min(0.0);
            parent_node.set_scroll_x_max(
                (content_bounds.width - bounds.width).max(0.0) as f64,
            );
        }
            ...
            && content_bounds.height > bounds.height
        {
            ...
            scrollbar.set_max_numeric_value(
                (content_bounds.height - bounds.height).max(0.0) as f64,
            );
...
mod tests {
    ...
    fn action_handler_queues_requests() {
        ...
        assert_eq!(pending[0].request.target, NodeId(1));
    }
    ...
    fn adapter_lifecycle() {
        ...
        with_event_loop(|event_loop| {
            ...
            let mut adapter =
                A11yAdapter::new(event_loop, window, id, "Test Window");
    ...
    fn announcements_coexist_with_widget_nodes() {
        let mut builder = TreeBuilder::new("Test Window")
            .with_announcements(&["Alert".to_owned()]);
    ...
    fn multiple_text_children_all_preserved() {
        ...
        let values: Vec<_> =
            label_nodes.iter().map(|(_, n)| n.value()).collect();
    ...
    impl Scrollable for MockScrollable {
        fn snap_to(
            &mut self,
            _: crate::core::widget::operation::scrollable::RelativeOffset<
                Option<f32>,
            >,
        ) {
        }
    ...
    fn nested_scrollable_offset_accumulates() {
        ...
        let (_, button_stored) =
            tree.node_map.get(&button_id).expect("button in node_map");
        ...
        let (_, sibling_stored) =
            tree.node_map.get(&sibling_id).expect("sibling in node_map");
